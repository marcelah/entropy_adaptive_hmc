...
def main(argv):
  ...
  if FLAGS.low_rank:
    ...
    def make_pre_cond_fn(params):
      ...
      momentum_distribution = MultivariateNormalPrecisionFactorLinearOperator(
  ...
  if FLAGS.low_rank:
    path = os.path.join(FLAGS.model_dir,
                    'data_set__{}_steps__{}_rank__{}_id__{}'.format(
                      flags.FLAGS.data_set, flags.FLAGS.num_leapfrog_steps,
                      flags.FLAGS.rank, flags.FLAGS.id))
  else:
    path = os.path.join(FLAGS.model_dir,
                    'data_set__{}_steps__{}_id__{}'.format(
                      flags.FLAGS.data_set, flags.FLAGS.num_leapfrog_steps,
                      flags.FLAGS.id))
