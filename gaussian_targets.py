...
flags.DEFINE_string("cov_type",
                     default='log_space_independent_easier',
...
def main(argv):
  ...
  if FLAGS.cov_type=='GP':
    ...
    pre_cond_scale_diag_init = tfd.Normal(
      loc = 0., scale = .1).sample(dims)/FLAGS.num_leapfrog_steps
  ...
  path=os.path.join(FLAGS.model_dir,
                    'covariance__{}_steps__{}_dims__{}_id__{}'.format(
                      flags.FLAGS.cov_type,flags.FLAGS.num_leapfrog_steps,
  ...
  if FLAGS.adaptation == 'grad':
    ...
    np.savetxt(os.path.join(path, 'entropy_weight_hmc_adapt.csv'), entropy_weight_hmc_adapt, delimiter=",")
