...
class SimpleLeapfrogIntegrator(LeapfrogIntegrator):
  ...
  def __init__(self, target_fn, step_sizes, num_steps):
    ...
    self._num_steps = tf.cast(num_steps, tf.int32)
  ...
  def __call__(self,
    ...
    Args:
      ...
      state_parts: Python `list` of `Tensor`s which collectively representing
        the state.
      #pre_cond_operator: A tf.linalg.LinearOperator subclass that represents the operation
      #  x mapsto C x where CC^T is the precision matrix corresponding to the kinetic_energy_fn
      target: Batch of scalar `Tensor` representing the target (i.e.,
        unnormalized log prob) evaluated at `state_parts`.
      target_grad_parts: Python `list` of `Tensor`s representing the gradient of
        `target` with respect to each of `state_parts`.
    ...
    with tf.name_scope(name or 'leapfrog_integrate'):
      ...
      # state_parts_array = tf.nest.map_structure(lambda p: tf.TensorArray(
      #   dtype = p.dtype, size = self._num_steps + 1), state_parts)
      # momentum_parts_array = tf.nest.map_structure(lambda p: tf.TensorArray(
      #   dtype = p.dtype, size = self._num_steps + 1), momentum_parts)
      # target_grads_parts_array = tf.nest.map_structure(lambda p: tf.TensorArray(
      #   dtype = p.dtype, size = self._num_steps + 1), momentum_parts)

      state_parts_array = tf.nest.map_structure(lambda p: tf.TensorArray(
        dtype = p.dtype, dynamic_size = True, size = 0), state_parts)
      momentum_parts_array = tf.nest.map_structure(lambda p: tf.TensorArray(
        dtype = p.dtype,dynamic_size = True, size = 0), momentum_parts)
      target_grads_parts_array = tf.nest.map_structure(lambda p: tf.TensorArray(
        dtype = p.dtype, dynamic_size = True, size = 0), momentum_parts)

      momentum_parts_array = tf.nest.map_structure(lambda ar, p: ar.write(0, p), momentum_parts_array, momentum_parts)
      target_grads_parts_array = tf.nest.map_structure(lambda ar, g: ar.write(0, g), target_grads_parts_array, target_grad_parts)
      state_parts_array = tf.nest.map_structure(lambda ar, g: ar.write(0, g), state_parts_array, state_parts)
      ...
      [
          ...
          body=lambda i, *args: [i + 1] + list(_one_step(  # pylint: disable=no-value-for-parameter,g-long-lambda
              i, self.target_fn, self.step_sizes, get_velocity_parts, *args)),
          loop_vars=[
      ...
      return (
          ...
          next_target,
          next_target_grad_parts,
          tf.nest.map_structure(lambda ar: ar.stack(), momentum_parts_array),
          tf.nest.map_structure(lambda ar: ar.stack(), target_grads_parts_array),
          tf.nest.map_structure(lambda ar: ar.stack(), state_parts_array)
      )
...
def _one_step(
  ...
  with tf.name_scope('leapfrog_integrate_one_step'):
    ...
    next_state_parts_for_grad = []
    for state_part, eps, velocity_part in zip(
        state_parts, step_sizes, velocity_parts):
      next_state_parts_for_grad.append(
          state_part + tf.cast(eps, state_part.dtype) *
          tf.cast(velocity_part, state_part.dtype))
    ...
    # save target grads and states in array
    target_grads_parts_array = tf.nest.map_structure(lambda ar, g: ar.write(i+1, g),
                          target_grads_parts_array, next_target_grad_parts)
    state_parts_array = tf.nest.map_structure(lambda ar, g: ar.write(i+1, g),
                          state_parts_array, next_state_parts)
    full_momentum_parts =  [
        v + tf.cast(eps/2, v.dtype) * tf.cast(g, v.dtype)  # pylint: disable=g-complex-comprehension
        for v, eps, g
        in zip(half_next_momentum_parts, step_sizes, next_target_grad_parts)]
    momentum_parts_array = tf.nest.map_structure(lambda ar, p: ar.write(i+1, p),
                          momentum_parts_array, full_momentum_parts)
